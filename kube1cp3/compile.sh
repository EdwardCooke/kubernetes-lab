...
certkey=`ssh kube1cp1.k8s.lan 'sudo kubeadm init phase upload-certs --upload-certs | sed -n "3p"' 2> /dev/null`
...
cloud-init devel make-mime -a build/user-data:cloud-config -a certs.sh:x-shellscript-per-once -a build/install-controlplane.sh:x-shellscript-per-once --force > cidata/user-data
