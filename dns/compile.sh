...
cloud-init devel make-mime -a build/user-data.source:cloud-config -a bind.sh:x-shellscript-per-once -a build/certs.sh:x-shellscript-per-once --force > cidata/user-data
cp meta-data cidata
