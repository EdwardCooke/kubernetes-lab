...
cloud-init devel make-mime -a cidata/user-data.source:cloud-config -a bind.sh:x-shellscript-per-once -a certs.sh:x-shellscript-per-once --force > cidata/user-data
