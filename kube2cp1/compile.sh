...
cloud-init devel make-mime -a cidata/user-data.source:cloud-config -a certs.sh:x-shellscript-per-once -a install-controlplane.sh:x-shellscript-per-once --force > cidata/user-data
