...
cloud-init devel make-mime -a build/user-data:cloud-config -a certs.sh:x-shellscript-per-once -a build/install-worker.sh:x-shellscript-per-once --force > cidata/user-data
